...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-white py-16 md:py-24"><div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(180deg, #fffaf1 0%, #fffaf1 45%, #fffaf1 100%)' }} /><div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6"><div className="mx-auto flex max-w-4xl flex-col items-center gap-5 text-center"><p className="text-xs font-bold tracking-[0.28em] text-[#D4AF37]">TRADER REVIEWS</p><h2 className="text-4xl md:text-6xl font-extrabold tracking-[-1.5px] text-black text-balance">The trusted choice for CK traders</h2><div className="flex flex-wrap items-center justify-center gap-3 text-sm font-medium text-black/70"><span>4.9 based on Trustpilot reviews</span><TrustStars /></div><a href="https://uk.trustpilot.com/review/ckcapital.co.uk" target="_blank" rel="noopener noreferrer" className="inline-flex h-12 items-center justify-center rounded-xl bg-[#D4AF37] px-6 text-sm font-extrabold text-black transition-colors hover:bg-[#F7D774] md:h-14 md:px-10">Read All Reviews on Trustpilot ↗</a></div><div className="mt-14 grid gap-4 md:grid-cols-4"><div className="group relative min-h-[410px] overflow-hidden rounded-2xl border border-black/10 bg-black/[0.04] md:col-span-1"><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,rgba(212,175,55,0.55),transparent_45%),linear-gradient(180deg,#fffaf1,#fffaf1)]" /><div className="relative z-10 flex h-full flex-col justify-end p-5"><div className="rounded-2xl border border-black/10 bg-white/70 p-4 backdrop-blur"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#D4AF37]">Video Story</p><p className="mt-2 text-lg font-bold text-black">Trader experience preview</p><p className="mt-1 text-sm text-black/60">Add CK testimonial video here when approved.</p></div></div></div><div className="rounded-2xl border border-[#D4AF37]/30 bg-[#D4AF37] p-5 text-black md:col-span-1"><p className="text-5xl font-extrabold">24/7</p><p className="mt-2 text-xs font-extrabold uppercase tracking-[0.2em] text-black/60">Support Available</p><div className="mt-8 h-24 rounded-xl bg-black/10 p-3"><div className="h-3 w-3/4 rounded-full bg-black/20" /><div className="mt-3 h-3 w-1/2 rounded-full bg-black/20" /><div className="mt-3 h-3 w-5/6 rounded-full bg-black/20" /></div></div>{REVIEW_CARDS.map((review) => <article key={review.quote} className={`flex min-h-[240px] flex-col justify-between rounded-2xl border p-5 ${review.tone === 'dark' ? 'border-black/10 bg-[#fffaf1] text-black' : review.tone === 'gold' ? 'border-[#D4AF37]/30 bg-[#F7D774] text-black' : 'border-black/10 bg-white text-black'}`}><div><TrustStars compact /><p className={`mt-4 text-sm leading-relaxed ${review.tone === 'dark' ? 'text-black/70' : 'text-black/75'}`}>&ldquo;{review.quote}&rdquo;</p></div><div className="mt-6 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-full bg-white text-sm font-extrabold text-[#D4AF37]">{review.name[0]}</span><div><p className="text-sm font-extrabold">{review.name}</p><p className={`text-xs ${review.tone === 'dark' ? 'text-black/55' : 'text-black/55'}`}>{review.country}</p></div></div><span className={`rounded-full px-3 py-1 text-[10px] font-bold ${review.tone === 'dark' ? 'bg-black/[0.04] text-black/70' : 'bg-black/10 text-black/65'}`}>{review.source}</span></div></article>)}</div><p className="mx-auto mt-8 max-w-3xl text-center text-xs text-black/45">Reviews reflect individual experiences and do not guarantee future results. CK Capital provides simulated trading evaluations only.</p></div></section>
