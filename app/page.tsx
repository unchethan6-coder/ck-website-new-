    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 pb-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 text-center">
          <div className="mx-auto mb-5 inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-2 text-xs font-semibold text-black/70 backdrop-blur"><span className="size-2 rounded-full bg-[#00B67A]" />Simulated trading evaluations with clear rules</div>
          <h1 className="hero-title text-[#111111] mb-6 text-balance">Trade with Clear Rules.<br className="hidden md:block" /> Earn Rewards for Consistency.</h1>
          <p className="mx-auto mb-8 max-w-3xl text-base md:text-xl leading-relaxed text-black/70">CK Capital provides structured simulated trading evaluations designed for disciplined traders who want flexible plans, transparent objectives, and a premium trading experience.</p>
          ...
          <div className="flex justify-center mb-12"><div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 rounded-2xl border border-black/10 bg-white/80 px-5 py-3 text-sm text-[#4B5563] shadow-sm backdrop-blur"><TrustStars compact /><span>Rated 4.9/5 on Trustpilot</span><a href="https://uk.trustpilot.com/review/ckcapital.co.uk" target="_blank" rel="noopener noreferrer" className="text-[#00B67A] font-semibold hover:underline">View Reviews →</a></div></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 max-w-3xl mx-auto mb-12"><StatCounter target={100} label="Max Reward Split" format="percent" /><StatCounter target={1200000} label="Max Simulated Capital" format="currency" /><div className="text-center"><div className="text-3xl md:text-4xl font-bold gradient-text">24/7</div><div className="text-sm text-[#4B5563] mt-2">Support Available</div></div></div>
        </div>
      </section>
      ...
      <section id="start-challenge" className="relative overflow-hidden py-16 md:py-24 scroll-mt-20 bg-[#050505]">
        ...
        <div className="relative z-10 mx-auto max-w-[1680px] px-4 md:px-8">
          <div className="mx-auto mb-8 max-w-3xl text-center md:mb-10">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm md:text-base text-white/65">Full-width, flexible pricing cards inspired by the FundingPips layout, using CK Capital account sizes and rules.</p>
          </div>
          ...
          {pricingView === 'cards' && (
            <div className="mx-auto pb-6">
              <div className="flex flex-wrap justify-center gap-6">
                {cards.map((card) => (
                  <article key={card.size} className={`relative flex min-h-[520px] w-full max-w-[290px] flex-[1_1_245px] flex-col overflow-hidden rounded-3xl border p-5 transition-all duration-300 hover:-translate-y-1 ${card.badge ? 'border-[#D4AF37] bg-white text-black shadow-[0_0_0_1px_rgba(212,175,55,0.25),0_30px_80px_rgba(212,175,55,0.22)]' : 'border-white/10 bg-white/[0.06] text-white hover:border-[#D4AF37]/60'}`}>
                    ...
                    <div className={`mb-5 rounded-2xl border p-4 ${card.badge ? 'border-black/10 bg-black/[0.03]' : 'border-white/10 bg-white/[0.05]'}`}><div className="flex items-end gap-2"><span className="text-4xl font-extrabold text-[#D4AF37]">{card.price}</span><span className={`pb-1 text-sm line-through ${card.badge ? 'text-black/40' : 'text-white/40'}`}>{card.oldPrice}</span></div><p className={`mt-1 text-xs ${card.badge ? 'text-black/55' : 'text-white/55'}`}>Limited promotional pricing</p></div>
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">{[['Phase 1 Target', card.features.phase1], ['Phase 2 Target', card.features.phase2], ['Max Daily Loss', card.features.maxDaily], ['Max Loss', card.features.maxLoss], ['Trading Period', card.features.period], ['Min Trading Days', card.features.minDays], ['Reward Split', card.features.rewardSplit], ['Consistency Rule', card.features.consistency]].map(([label, value]) => (<div key={label} className="flex items-center justify-between gap-4 border-b border-current/10 pb-2 last:border-0"><span className={card.badge ? 'text-black/55' : 'text-white/55'}>{label}</span><span className="text-right font-bold">{value}</span></div>))}</div>
