    ...
    <div className="min-h-screen bg-background">
      ...
      <section id="start-challenge" className="relative overflow-hidden py-16 md:py-24 scroll-mt-20 bg-[#050505]">
        ...
        <div className="relative z-10 mx-auto max-w-[1680px] px-4 md:px-8">
          ...
          {pricingView === 'cards' && (<div className="mx-auto pb-6"><div className="flex flex-wrap justify-center gap-6">{cards.map((card) => (<article key={card.size} className={`relative flex min-h-[520px] w-full max-w-[300px] flex-[1_1_260px] flex-col overflow-hidden rounded-3xl border p-5 transition-all duration-300 hover:-translate-y-1 ${card.badge ? 'border-[#D4AF37] bg-white text-black shadow-[0_0_0_1px_rgba(212,175,55,0.25),0_30px_80px_rgba(212,175,55,0.22)]' : 'border-white/10 bg-white/[0.06] text-white hover:border-[#D4AF37]/60'}`}>{card.badge && <div className="absolute right-4 top-4 rounded-full bg-[#D4AF37] px-3 py-1 text-[10px] font-extrabold tracking-wide text-black">{card.badge}</div>}<p className={`mb-2 text-xs font-bold uppercase tracking-[0.18em] ${card.badge ? 'text-black/45' : 'text-white/45'}`}>Account Size</p><h3 className="mb-4 text-4xl font-extrabold tracking-tight">{card.size}</h3><div className={`mb-5 overflow-hidden rounded-2xl border p-4 ${card.badge ? 'border-black/10 bg-black/[0.03]' : 'border-white/10 bg-white/[0.05]'}`}><div className="flex min-w-0 flex-col items-start"><span className="max-w-full truncate text-[38px] font-extrabold leading-none text-[#D4AF37]">{card.price}</span><span className={`mt-2 block max-w-full truncate text-xs line-through ${card.badge ? 'text-black/40' : 'text-white/40'}`}>{card.oldPrice}</span></div><p className={`mt-2 text-xs ${card.badge ? 'text-black/55' : 'text-white/55'}`}>Limited promotional pricing</p></div><Link href="https://app.ckcapital.co.uk/signup" target="_blank" rel="noopener noreferrer" className={`mb-5 flex h-12 w-full items-center justify-center rounded-xl text-sm font-extrabold transition-all ${card.badge ? 'bg-black text-white hover:bg-black/85' : 'bg-[#D4AF37] text-black hover:bg-[#F7D774]'}`}>Start Challenge</Link><div className="grid grid-cols-1 gap-3 text-sm">{[['Phase 1 Target', card.features.phase1], ['Phase 2 Target', card.features.phase2], ['Max Daily Loss', card.features.maxDaily], ['Max Loss', card.features.maxLoss], ['Trading Period', card.features.period], ['Min Trading Days', card.features.minDays], ['Reward Split', card.features.rewardSplit], ['Consistency Rule', card.features.consistency]].map(([label, value]) => (<div key={label} className="flex items-center justify-between gap-4 border-b border-current/10 pb-2 last:border-0"><span className={card.badge ? 'text-black/55' : 'text-white/55'}>{label}</span><span className="text-right font-bold">{value}</span></div>))}</div><button type="button" onClick={() => navigator.clipboard.writeText(`JUN70-${card.size.replace('$', '').replace('K', '')}`)} className={`mt-auto rounded-xl border border-dashed px-3 py-3 text-xs font-extrabold transition-all ${card.badge ? 'border-black/20 bg-black/[0.04] text-black hover:bg-black/[0.08]' : 'border-[#D4AF37]/40 bg-[#D4AF37]/10 text-[#F7D774] hover:bg-[#D4AF37]/20'}`}>Copy Code: JUN70-{card.size.replace('$', '').replace('K', '')}</button></article>))}</div></div>)}
