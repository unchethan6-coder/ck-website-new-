...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section id="start-challenge" className="relative overflow-hidden py-16 md:py-24 scroll-mt-20 bg-white">
        ...
        <div className="relative z-10 mx-auto max-w-[1680px] px-4 md:px-8">
          <div className="mx-auto mb-8 max-w-3xl text-center md:mb-10">
            ...
            <h2 className="text-3xl font-extrabold tracking-[-1px] text-black text-balance md:text-5xl lg:text-6xl">Choose your account size. Start your evaluation.</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm md:text-base text-black/65">Full-width, flexible pricing cards inspired by the FundingPips layout, using CK Capital account sizes and rules.</p>
          </div>

          <div className="mb-8 flex justify-center overflow-x-auto pb-2">
            <div className="flex w-max gap-2 rounded-2xl border border-black/10 bg-black/[0.03] p-2 backdrop-blur md:flex-wrap md:justify-center">
              {CHALLENGE_TABS.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setSelectedChallengeType(tab.id)}
                  className={`min-w-[132px] rounded-xl px-4 py-3 text-left transition-all ${selectedChallengeType === tab.id ? 'bg-white text-black shadow-[0_12px_35px_rgba(212,175,55,0.25)]' : 'text-black/70 hover:bg-white/10 hover:text-black'}`}
                >
          ...
          <div className="mb-10 flex justify-center">
            <div className="inline-flex rounded-full border border-black/10 bg-white p-1">
              <button onClick={() => setPricingView('cards')} className={`px-6 py-2 rounded-full text-sm font-semibold transition-all ${pricingView === 'cards' ? 'bg-[#D4AF37] text-black' : 'text-black/70'}`}>Cards</button>
              <button onClick={() => setPricingView('table')} className={`px-6 py-2 rounded-full text-sm font-semibold transition-all ${pricingView === 'table' ? 'bg-[#D4AF37] text-black' : 'text-black/70'}`}>Table</button>
            </div>
          </div>
                ...
                <div className="flex flex-wrap justify-center gap-6 xl:flex-nowrap xl:gap-4 2xl:gap-5">
                  {cards.map((card) => (
                    <article key={card.size} className={`relative flex min-h-[500px] w-full max-w-[300px] flex-[1_1_260px] flex-col overflow-hidden rounded-3xl border p-5 transition-all duration-300 hover:-translate-y-1 xl:max-w-[250px] xl:flex-[1_1_0] xl:p-4 2xl:max-w-[270px] 2xl:p-5 ${card.badge ? 'border-[#D4AF37] bg-white text-black shadow-[0_0_0_1px_rgba(212,175,55,0.25),0_30px_80px_rgba(212,175,55,0.22)]' : 'border-black/10 bg-white text-black hover:border-[#D4AF37]/60'}`}>
                      ...
                      <p className={`mb-2 text-xs font-bold uppercase tracking-[0.18em] ${card.badge ? 'text-black/45' : 'text-black/45'}`}>Account Size</p>
                      <h3 className="mb-4 text-4xl font-extrabold tracking-tight xl:text-3xl 2xl:text-4xl">{card.size}</h3>
                      <div className={`mb-5 overflow-hidden rounded-2xl border p-4 xl:p-3 2xl:p-4 ${card.badge ? 'border-black/10 bg-black/[0.03]' : 'border-black/10 bg-black/[0.03]'}`}>
                        <div className="flex min-w-0 flex-col items-start">
                          <span className="max-w-full truncate text-[38px] font-extrabold leading-none text-[#D4AF37] xl:text-[30px] 2xl:text-[36px]">{card.price}</span>
                          <span className={`mt-2 block max-w-full truncate text-xs line-through ${card.badge ? 'text-black/40' : 'text-black/40'}`}>{card.oldPrice}</span>
                        </div>
                        <p className={`mt-2 text-xs ${card.badge ? 'text-black/55' : 'text-black/55'}`}>Limited promotional pricing</p>
                      </div>
                      <Link href="https://app.ckcapital.co.uk/signup" target="_blank" rel="noopener noreferrer" className={`mb-5 flex h-12 w-full items-center justify-center rounded-xl text-sm font-extrabold transition-all ${card.badge ? 'bg-[#A87B0B] text-foreground hover:bg-[#8a6309]' : 'bg-[#D4AF37] text-black hover:bg-[#F7D774]'}`}>Start Challenge</Link>
                      <div className="grid grid-cols-1 gap-3 text-sm xl:text-xs 2xl:text-sm">
                        {[['Phase 1 Target', card.features.phase1], ['Phase 2 Target', card.features.phase2], ['Max Daily Loss', card.features.maxDaily], ['Max Loss', card.features.maxLoss], ['Trading Period', card.features.period], ['Min Trading Days', card.features.minDays], ['Reward Split', card.features.rewardSplit], ['Consistency Rule', card.features.consistency]].map(([label, value]) => (
                          <div key={label} className="flex items-center justify-between gap-3 border-b border-current/10 pb-2 last:border-0">
                            <span className={card.badge ? 'text-black/55' : 'text-black/55'}>{label}</span>
                            <span className="text-right font-bold">{value}</span>
                          </div>
                        ))}
                      </div>
                      <button type="button" onClick={() => navigator.clipboard.writeText(`JUN70-${card.size.replace('$', '').replace('K', '')}`)} className={`mt-auto rounded-xl border border-dashed px-3 py-3 text-xs font-extrabold transition-all xl:px-2 xl:text-[11px] 2xl:text-xs ${card.badge ? 'border-black/20 bg-black/[0.04] text-black hover:bg-black/[0.08]' : 'border-[#D4AF37]/40 bg-[#D4AF37]/10 text-[#F7D774] hover:bg-[#D4AF37]/20'}`}>Copy Code: JUN70-{card.size.replace('$', '').replace('K', '')}</button>
                    </article>
                  ))}
                </div>
              </div>
          ...
          <p className="mx-auto mt-8 max-w-3xl text-center text-xs md:text-sm text-black/45">CK Capital programs use demo accounts with fictitious funds for simulated trading evaluation only. Program terms, rules, and eligibility apply.</p>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-white py-16 md:py-24">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6">
          <div className="mx-auto flex max-w-4xl flex-col items-center gap-5 text-center">
            ...
            <h2 className="text-4xl md:text-6xl font-extrabold tracking-[-1.5px] text-black text-balance">The trusted choice for CK traders</h2>
            <div className="flex flex-wrap items-center justify-center gap-3 text-sm font-medium text-black/70">
              <span>4.9 based on Trustpilot reviews</span>
              <TrustStars />
            </div>
            <a href="https://uk.trustpilot.com/review/ckcapital.co.uk" target="_blank" rel="noopener noreferrer" className="inline-flex h-12 items-center justify-center rounded-xl bg-[#D4AF37] px-6 text-sm font-extrabold text-black transition-colors hover:bg-[#F7D774] md:h-14 md:px-10">Read All Reviews on Trustpilot ↗</a>
          </div>
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-4">
            ...
            {REVIEW_CARDS.map((review) => (
              <article key={review.quote} className={`flex min-h-[240px] flex-col justify-between rounded-2xl border p-5 ${review.tone === 'dark' ? 'border-black/10 bg-[#fffaf1] text-black' : review.tone === 'gold' ? 'border-[#D4AF37]/30 bg-[#F7D774] text-black' : 'border-black/10 bg-white text-black'}`}>
                <div>
                  <TrustStars compact />
                  <p className={`mt-4 text-sm leading-relaxed ${review.tone === 'dark' ? 'text-black/70' : 'text-black/75'}`}>&ldquo;{review.quote}&rdquo;</p>
                </div>
