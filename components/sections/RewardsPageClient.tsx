          ...
          return realReviews.length > 0 ? (
            <section className="py-20 md:py-28" data-od-id="reward-reviews">
              <Container>
                ...
                <div className="mt-12 grid gap-4 md:grid-cols-3">{realReviews.slice(0, reviewCount).map((review) => <article key={review.id} className="flex min-h-56 flex-col justify-between rounded-xl border border-foreground/10 bg-foreground/[0.035] p-6" data-od-id={`reward-review-${review.id}`}><div><div className="flex items-center justify-between"><div className="flex items-center gap-1 text-primary">{Array.from({ length: Math.min(review.rating || 5, 5) }).map((_, i) => <Sparkles key={i} size={13} fill="currentColor" />)}</div>{review.source && <span className="inline-flex items-center gap-1 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-foreground/40"><span className="h-1.5 w-1.5 rounded-full bg-current" />{review.source}</span>}</div><blockquote className="mt-4 text-sm leading-7 text-foreground/70">"{review.summary}"</blockquote></div><div className="mt-6 flex items-center gap-3 border-t border-foreground/[0.08] pt-4"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/15 text-xs font-bold text-primary">{initials(review.authorName)}</span><div><p className="text-sm font-semibold text-foreground">{review.authorName}</p><p className="text-xs text-foreground/35">{review.countryName || ""}</p></div></div></article>)}</div>{reviewCount < realReviews.length ? <div className="mt-8 text-center"><button type="button" onClick={() => setReviewCount((count) => Math.min(count + 3, realReviews.length))} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-primary/35 px-5 text-xs font-bold uppercase tracking-[0.14em] text-primary hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Read more reviews <ChevronDown size={15} /></button></div> : null}
              </Container>
            </section>
          ) : null;
        })()}
