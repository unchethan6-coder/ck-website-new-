  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 px-4 bg-gradient-to-b from-white to-gray-50">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="bg-card rounded-lg shadow-sm p-4 sm:p-6 md:p-8 border border-border">
          ...
          <div className="flex gap-2 sm:gap-3 justify-center mb-6 sm:mb-8 flex-wrap">
            <button
              ...
              className={`px-3 sm:px-6 py-1.5 sm:py-2 rounded-lg border-2 font-semibold transition-all text-xs sm:text-sm md:text-base ${
                challengeType === '1step'
                  ? 'border-yellow-400 bg-white text-foreground'
            ...
            <button
              ...
              className={`px-3 sm:px-6 py-1.5 sm:py-2 rounded-lg border-2 font-semibold transition-all text-xs sm:text-sm md:text-base ${
                challengeType === '2step'
                  ? 'bg-primary border-yellow-400 text-foreground'
                  : 'border-gray-300 text-muted-foreground hover:border-gray-400'
              }`}
            >
              2 Step
            ...
            <button
              ...
              className={`px-3 sm:px-6 py-1.5 sm:py-2 rounded-lg border-2 font-semibold transition-all text-xs sm:text-sm md:text-base ${
                challengeType === 'instant'
                  ? 'border-gray-900 bg-gray-900 text-foreground'
                  : 'border-gray-300 text-muted-foreground hover:border-gray-400'
              }`}
            >
              Instant Funding
          ...
          <div className="mb-6 sm:mb-8 space-y-3 sm:space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6">
              <span className="font-semibold text-foreground text-sm sm:text-base flex-shrink-0">Select Preference:</span>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6">
              <span className="font-semibold text-foreground text-sm sm:text-base flex-shrink-0">Select Platform:</span>
              <div className="flex gap-3 sm:gap-6 flex-wrap">
          ...
          <div className="mb-6 sm:mb-8">
            <p className="font-semibold text-foreground mb-3 text-sm sm:text-base">Challenge Size:</p>
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr className="border-b-2 border-gray-300">
                  <td className="py-2 sm:py-3 px-2 sm:px-4 font-semibold text-foreground text-left bg-muted"></td>
                  <td className="py-2 sm:py-3 px-2 sm:px-4 font-semibold text-foreground text-center bg-muted">
                    <div className="text-xs text-muted-foreground mb-0.5 sm:mb-1">STEP 1</div>
                    <div className="text-xs sm:text-sm">{step1Label}</div>
                  </td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 font-semibold text-foreground text-center bg-muted">
                      <div className="text-xs text-muted-foreground mb-0.5 sm:mb-1">STEP 2</div>
                      <div className="text-xs sm:text-sm">{step2Label}</div>
                    </td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 font-semibold text-foreground text-center bg-muted">
                    <div className="text-xs text-muted-foreground mb-0.5 sm:mb-1">STEP {challengeType === 'instant' ? '2' : '3'}</div>
              ...
              <tbody>
                <tr className="border-b border-border">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.tradingPeriod}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.tradingPeriod}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.tradingPeriod}</td>
                ...
                <tr className="border-b border-border">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.profitTarget}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.profitTarget}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">x</td>
                </tr>
                <tr className="border-b border-border">
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-700 font-medium">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.dailyLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.dailyLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.dailyLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                ...
                <tr className="border-b border-border">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.maxLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.maxLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted text-xs sm:text-sm">${(parseFloat(currentRules.maxLoss.replace('$', '')) * (accountSize / 10000)).toLocaleString()}</td>
                ...
                <tr className="border-b border-border">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.minTradingDays}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.minTradingDays}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">x</td>
                ...
                <tr className="border-b border-border">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.consistency}</td>
                  {challengeType !== 'instant' && (
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">{currentRules.consistency}</td>
                  )}
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">x</td>
                ...
                <tr>
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-center bg-muted">
        ...
        {/* Disclaimer */}
        <p className="text-xs sm:text-sm text-muted-foreground text-center mt-6 sm:mt-8 leading-relaxed px-2">
